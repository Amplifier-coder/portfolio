...
// Personalize the accent without requiring storage access.
const accentButtons = [...document.querySelectorAll('[data-accent]')];
function setAccent(accent) {
  if (!['mint', 'violet', 'amber'].includes(accent)) return;
  document.documentElement.dataset.accent = accent;
  accentButtons.forEach(button => button.setAttribute('aria-pressed', String(button.dataset.accent === accent)));
}
try { setAccent(localStorage.getItem('portfolio-accent') || 'mint'); } catch { setAccent('mint'); }
accentButtons.forEach(button => button.addEventListener('click', () => {
  setAccent(button.dataset.accent);
  try { localStorage.setItem('portfolio-accent', button.dataset.accent); } catch { /* Works without persistence. */ }
}));
document.querySelector('.palette').hidden = false;
